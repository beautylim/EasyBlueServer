...
/**
 * Servlet implementation class EditBornTimeServlet
 */
@WebServlet("/EditBornTimeServlet")
public class EditBornTimeServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EditBornTimeServlet() {
        super();
	      ...
	      String accessToken = request.getParameter("accessToken").trim();
			...
			try {
					...
					if(msgAccessToken.toString().trim().length() == 0){
						...
						String bornTime = request.getParameter("bornTime").toString().trim();
							String updateTime = request.getParameter("updateTime").toString().trim();
							String sqlUser = "update UserInfo set bornDate = ?,updateTime = ? where uuid = ?";
							Object[] parUSer = new Object[]{bornTime,updateTime,msgAccessToken.toString().trim()};
