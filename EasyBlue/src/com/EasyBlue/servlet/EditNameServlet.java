			...
			try {
				while(rsAccessToken.next()){
					...
					if(msgAccessToken.toString().trim().length() == 0){
						...
						String updteTime = request.getParameter("updateTime").toString().trim();
						String sqlUser = "update UserInfo set name = ?,updateTime = ? where uuid = ?";
						Object[] parUSer = new Object[]{name,updteTime,msgAccessToken.toString().trim()};
