			...
			try {
				while(rsAccessToken.next()){
					...
					if(msgAccessToken.toString().trim().length() == 0){
						...
						String placeId = request.getParameter("placeId").toString().trim();
						String updateTime = request.getParameter("updateTime").toString().trim();
						String sqlUser = "update UserInfo set placeId = ?,updateTime = ? where uuid = ?";
						Object[] parUSer = new Object[]{placeId,updateTime,msgAccessToken.toString().trim()};
